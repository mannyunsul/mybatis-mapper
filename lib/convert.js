...
var convertInclude = function (children, param, namespace, myBatisMapper) {
  ...
  try {
    ...
    // cross-namespace include 지원
    var targetNamespace = namespace
    var targetRefid = refid

    if (refid.includes(".")) {
      var parts = refid.split(".")
      targetNamespace = parts[0]
      targetRefid = parts.slice(1).join(".") // 혹시 id에 점이 있을 가능성 대비
    }

    var statement = ""
    var nodes = myBatisMapper?.[targetNamespace]?.[targetRefid]
    if (!nodes) {
      throw new Error(
        `Include refid not found: ${targetNamespace}.${targetRefid} (from ${namespace}.${refid})`
      )
    }

    for (var i = 0, node; (node = nodes[i]); i++) {
      statement += convertChildren(node, param, targetNamespace, myBatisMapper)
    }
  } catch (err) {
